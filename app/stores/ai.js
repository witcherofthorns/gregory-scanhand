            ...
            try{
                ...
                if(response.status == 200){
                    ...
                    if(status == "processing"){
                        modalStore.open('text', {
                            skipable: true,
                            title: 'Ожидайте',
                            text: 'Вы уже отправили свои фото для предсказания, дождитесь результата, обычно это может занимать от 10 до 30 секунд',
                            buttonClose: true,
                ...
                if(leftOk.ok && rightOk.ok){
                    ...
                    modalStore.open('text', {
                        skipable: true,
                        title: 'Отправлено',
                        text: 'Совсем скоро начнём предсказание по вашим ладоням! Дождитесь результата, обычно это занимает не больше 10-30 секунд',
                        buttonClose: true,
                        buttonCloseText: 'Хорошо'
                    })
        ...
        modalOpen(){
            ...
            if(this.result.result.status == "reshot"){
                modalStore.open('text', {
                    ...
                    text: 'К сожалению, мы вынуждены попросить вас сделать фото повторно, так как ваши отправленные фото плохого качества или размыты',
                    buttonClose: true,
